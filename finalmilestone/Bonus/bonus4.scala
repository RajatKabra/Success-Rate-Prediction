...
val data = pipedata.select('dog_SubStatusCode,'foodfeatures, 'earcleaning, 'attendsclasses, 'attendshomeswitches, 'nailcutting, 'behaveswellclass,'cangivepills, 'stealsfood, 'jumponpeople, 'raidsgarbage, 'age, 'dog_sex, 'trafficfear,'relationship_description, 'health, 'quietincrate, 'housemanners, 'leftunattended, 'onfurniture,'barksexcessively,'ExerciseType)
...
val assembler = new VectorAssembler().setInputCols(Array("excercisefeatures","foodfeatures","earcleaning","attendsclasses", "attendshomeswitches", "nailcutting", "behaveswellclass")).setOutputCol("features")
